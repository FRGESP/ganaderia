        ...
        IF(MESESIN < 16) THEN
            ...
            IF(ESTADOIN = 3) THEN -- SI ESTÁ MUERTO NO LO AGREGA A NINGUN CORRAL
                ...
                ELSE
                    ...
                    UPDATE CORRAL SET Cantidad = VARTEMP, REEMO = REEMOIN WHERE IdCorral = CORRALOPTION; -- ACTUALIZANDO LA CANTIDAD EN LOS CORRALES
            end if;
...
CREATE PROCEDURE GETDIETAINFO(IN DIETAIN INT)
    BEGIN
       ...
       SELECT Corral, Cantidad, SUMA AS Suma FROM CORRAL WHERE Dieta = DIETAIN;
       SELECT A.IdArticulo AS Id, A.Articulo, CONCAT(DA.Cantidad,' ', A.Unidad) AS Cantidad, DA.Cantidad AS CantidadNumber FROM DIETA_ARTICULO AS DA INNER JOIN ARTICULO AS A on DA.Articulo = A.IdArticulo WHERE DA.Dieta = DIETAIN;
    END;
...
CREATE PROCEDURE GETDIETAOPTIONS()
    BEGIN
       ...
       SELECT IdCorral, Corral FROM CORRAL WHERE Dieta IS NULL;
    END;
