...
CREATE VIEW EMPLEADOSPAGE AS
    SELECT U.IdUsuario, E.IdEmpleado AS Id, CONCAT(P.Nombre, ' ', P.ApellidoPaterno, ' ', P.ApellidoMaterno) AS Nombre, P.Edad, P.Telefono, R.Rol, E.Sueldo, ES.Estatus, ES.IdEstatus FROM EMPLEADO AS E INNER JOIN PERSONA AS P ON E.IdPersona = P.IdPersona INNER JOIN ESTATUS AS ES ON E.Estatus = ES.IdEstatus INNER JOIN ROL AS R ON E.Rol = R.IdRol INNER JOIN USUARIO U on E.IdEmpleado = U.IdEmpleado ORDER BY E.Fecha ASC;
...
CREATE PROCEDURE SP_EMPLEADOSVISTA(IN IDEMP INT)
    BEGIN
        SELECT Id, Nombre, Edad, Telefono, Rol, Sueldo, Estatus FROM EMPLEADOSPAGE WHERE IdUsuario != IDEMP AND IdEstatus != 4;
    END;
...
CREATE PROCEDURE SP_DELEETEMPLEADO(IN IDINT INT)
    BEGIN
        ...
        UPDATE EMPLEADO SET Estatus = 4 WHERE IdEmpleado = IDINT;
...
CREATE PROCEDURE GETDIETAOPTIONS()
    BEGIN
       SELECT IdArticulo AS Id, Articulo FROM ARTICULO WHERE Estatus != 0 AND Categoria = 'Almacen';
...
DROP PROCEDURE IF EXISTS SP_GETCORRALVENTADATA;
CREATE PROCEDURE SP_GETCORRALVENTADATA(IN CORRALIN INT)
    BEGIN
        ...
        SELECT FORMAT(INVERSIONVAR,2) AS Inversion, FORMAT(PRECIOVENTA,2) AS Precio, PRECIOVENTA AS PrecioNumber,FORMAT((PRECIOVENTA-INVERSIONVAR),2) AS Ganancia, REEMOVAR AS REEMO, M.IdMotivo, M.Motivo FROM GUIAENTRADA AS GU INNER JOIN MOTIVO AS M ON GU.Motivo = M.IdMotivo WHERE GU.REEMO = REEMOVAR;

        SELECT G.Arete, C.Corral, G.Sexo, G.Meses, G.Clasificacion, G.Peso AS Peso, FORMAT(G.PrecioCompra,2) AS PrecioCompra, GETPRECIOSUGERIDO(G.Arete) AS PrecioSugerido FROM GANADO AS G INNER JOIN CORRAL AS C ON G.Corral = C.IdCorral WHERE G.REEMO = REEMOVAR AND Estado != 3;
    END;
