...
CREATE PROCEDURE GETDIETAOPTIONS()
    BEGIN
       ...
       SELECT IdCorral AS Id, Corral FROM CORRAL WHERE Dieta IS NULL AND Estatus != 0;
    END;
...
CREATE PROCEDURE SP_GETCORRALESVENTA()
    BEGIN
       SELECT IdCorral AS Id, Corral FROM CORRAL AS C INNER JOIN GUIA AS G ON C.REEMO = G.REEMO WHERE C.Dieta = 1 OR C.Dieta = 5 AND C.Estatus != 3;
    END;

CALL SP_GETCORRALESVENTA;
