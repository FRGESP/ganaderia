...
CREATE PROCEDURE SP_GETARTICULOS(IN NOMBREIN VARCHAR(100))
    BEGIN
        SELECT A.IdArticulo AS Id, A.Articulo, (CONCAT(SUM(L.Cantidad),' ', A.Unidad)) AS Existencia, FORMAT(SUM(L.Precio),2) AS Inversion FROM LOTE AS L INNER JOIN ARTICULO AS A on L.Articulo = A.IdArticulo  WHERE A.Categoria = 'Almacen' AND A.Articulo LIKE CONCAT('%',NOMBREIN,'%') GROUP BY A.Articulo, A.Unidad, A.IdArticulo;
    END;

DROP PROCEDURE SP_ADDARTICULO;
