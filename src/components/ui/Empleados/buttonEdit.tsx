...
function ButtonEditEmpleados({id}: ButtonEditEmpleadosProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <button className=" border border-[#ececec] h-10 w-10 flex items-center justify-center rounded-md hover:bg-[#ececec]" onClick={() => { getInfo(); }}>
    ...
    <Suspense fallback={<div>Cargando Pa</div>}>
      ...
      <form className=' rounded-md mb-4' onSubmit={handlesubmit} ref={form}>
        ...
        <input value={empleado.Edad} type="number" id="Edad" name="Edad" placeholder='Edad' onChange={handleChange} className='shadow appearance-none border rounded w-full py-2 px-3 text-black border-black'/>
        <label htmlFor="Telefono" className='block text-gray-700 text-sm font-bold my-2' >Teléfono</label>
        <input value={empleado.Telefono} type="number" id="Telefono" name="Telefono" placeholder='Telefono' onChange={handleChange} className='shadow appearance-none border rounded w-full py-2 px-3 text-black border-black'/>
        <label htmlFor="Sueldo" className='block text-gray-700 text-sm font-bold my-2'>Sueldo</label>
        <input value={empleado.Sueldo} type="number" id="Sueldo" name="Sueldo" placeholder='Sueldo' onChange={handleChange} className='shadow appearance-none border rounded w-full py-2 px-3 text-black border-black '/>
        <label htmlFor="Rol" className='block text-gray-700 text-sm font-bold my-2' >Rol</label>
        <select name="Rol" id="Rol" value={empleado.Rol} onChange={handleChange} className="w-full border border-black py-2 rounded-md">
        ...
        <select name="Estatus" id="Estatus" value={empleado.Estatus} onChange={handleChange} className="w-full border border-black py-2 rounded-md">
          <option value={"1"}>Activo</option>
          <option value={"2"}>Despedido</option>
          <option value={"3"}>Suspendido</option>
        </select>
      
        <button className='bg-acento hover:bg-acentohover w-full p-4 text-md text-white rounded-full py-3 mt-4'>Actualizar</button>
      </form>
        </div>
    </Suspense>
      </DialogContent>
